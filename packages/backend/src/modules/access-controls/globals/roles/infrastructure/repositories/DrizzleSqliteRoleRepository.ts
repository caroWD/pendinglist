...
import { PermissionMapper } from '../../../permissions/infrastructure/services/PermissionMapper.ts'
import type { Role } from '../../domain/models/Role.ts'
import type {
  BaseInsert,
  BaseSelect,
} from '../../../../../primitives/bases/infrastructure/baseSchema.ts'
import { RoleMapper } from '../services/RoleMapper.ts'
  ...
  async toggle(id: RootId): Promise<void> {
    ...
    const { rowsAffected } = await this._sqlite
      ...
      .set({
        ...
        archivedAt: role.state === 'active' ? getTemporalNow().toJSON() : null,
      })
      .where(eq(rolesTableSqlite.id, id.value))
